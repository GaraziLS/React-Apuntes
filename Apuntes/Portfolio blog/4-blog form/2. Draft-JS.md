...
No that we have that we'll c0ome to the actual remove link tag and pass in the handler.
...
We're going to copy the apiAction and oaste it into the axios call so it's dynamic, but first we'll update the ComponentWillMount because there's a small bug.
...
Since the edit mode is false, now the content will be rendered as usual, without forms. Now we'll pass that function as a prop to the blogForm, so the blog-form.js file receives it.
